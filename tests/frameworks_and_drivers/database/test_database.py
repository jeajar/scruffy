...
from scruffy.frameworks_and_drivers.database.database import (
    _migrate_rename_overseerr_settings_keys,
)
...
def test_migrate_renames_overseerr_keys_to_seer(in_memory_engine):
    """Existing services.overseerr_* rows are renamed to services.seer_*."""
    ...
    _migrate_rename_overseerr_settings_keys(in_memory_engine)
    ...
    with Session(in_memory_engine) as session:
        ...
        assert _value(session, "services.seer_url") == "http://old:5055"
        assert _value(session, "services.seer_api_key") == "old-key"


def test_migrate_keeps_existing_seer_value_and_drops_stale_overseerr_row(
    in_memory_engine,
):
    """If services.seer_url already has a value, it wins and the old row is dropped."""
    with Session(in_memory_engine) as session:
        session.add(
            SettingsModel(key="services.overseerr_url", value="http://old:5055")
        )
    ...
    with Session(in_memory_engine) as session:
        assert session.get(SettingsModel, "services.overseerr_url") is None
        assert _value(session, "services.seer_url") == "http://new:5055"
