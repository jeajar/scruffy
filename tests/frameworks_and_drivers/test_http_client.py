...
@pytest.mark.asyncio
async def test_get_with_headers(client):
...
@pytest.mark.asyncio
async def test_delete_with_headers(client):
    ...
    with respx.mock:
        ...
        await client.delete("http://test.com/api/resource/1", headers={"X-API-Key": "test-key"})

        assert respx.calls.last.request.headers["X-API-Key"] == "test-key"


@pytest.mark.asyncio
async def test_delete_with_params(client):
    ...
    with respx.mock:
        ...
        await client.delete("http://test.com/api/resource/1", params={"deleteFiles": "true"})

        assert "deleteFiles=true" in str(respx.calls.last.request.url)


@pytest.mark.asyncio
async def test_delete_raises_on_error(client):
...
async def test_put_success(client):
    ...
    with respx.mock:
        ...
        result = await client.put("http://test.com/api/resource/1", json={"name": "test"})

        assert result == {"id": 1, "updated": True}


@pytest.mark.asyncio
async def test_put_with_json(client):
    ...
    with respx.mock:
        ...
        await client.put("http://test.com/api/resource/1", json={"name": "test", "value": 123})
...
@pytest.mark.asyncio
async def test_put_with_headers(client):
    ...
    with respx.mock:
        ...
        await client.put("http://test.com/api/resource/1", headers={"Content-Type": "application/json"})

        assert respx.calls.last.request.headers["Content-Type"] == "application/json"


@pytest.mark.asyncio
async def test_put_raises_on_error(client):
    ...
    with respx.mock:
        ...
        with pytest.raises(httpx.HTTPStatusError):
            await client.put("http://test.com/api/resource/1", json={"name": "test"})
