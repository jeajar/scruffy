...
class TestLokiHandler:
    ...
    def test_close_closes_client(self, handler, mock_client, log_record):
        """Test that close() closes the HTTP client and prevents further emits."""
        mock_response = MagicMock()
        mock_response.raise_for_status = MagicMock()
        mock_client.post.return_value = mock_response

        # First emit to create the client
        handler.emit(log_record)
        mock_client.post.assert_called_once()
        ...
        # Reset mock and try to emit again
    ...
    def test_emit_recreates_closed_client(self, log_record):
        ...
        with patch(
            ...
            mock_response = MagicMock()
            mock_response.raise_for_status = MagicMock()
            first_client.post.return_value = mock_response

            # Second client - replacement
            second_client = MagicMock()
            second_client.is_closed = False
            second_client.post.return_value = mock_response

            mock.side_effect = [first_client, second_client]

            handler = LokiHandler(url="http://loki:3100/loki/api/v1/push")

            # First emit creates and uses first client
            handler.emit(log_record)
            first_client.post.assert_called_once()

            # Simulate client being closed externally
            first_client.is_closed = True

            # Second emit should create new client
            handler.emit(log_record)
            second_client.post.assert_called_once()

    def test_emit_skipped_when_handler_closed(self, handler, mock_client, log_record):
        """Test that emit is skipped when handler is explicitly closed."""
