...
def test_initialization_disabled():
    ...
    with patch("scruffy.frameworks_and_drivers.email.email_client.settings") as mock_settings:
...
def test_initialization_enabled(mock_settings, mock_fastmail, mock_template_env):
...
def test_initialization_no_credentials():
    ...
    with patch("scruffy.frameworks_and_drivers.email.email_client.settings") as mock_settings:
        ...
        with patch("scruffy.frameworks_and_drivers.email.email_client.FastMail"):
            with patch("scruffy.frameworks_and_drivers.email.email_client.Environment"):
                client = EmailClient()
...
@pytest.mark.asyncio
async def test_send_deletion_notice(mock_settings, mock_fastmail, mock_template_env):
    """Test send_deletion_notice sends email."""
    client = EmailClient()

    await client.send_deletion_notice("test@test.com", "Test Movie", "poster.jpg", days_left=0)
...
@pytest.mark.asyncio
async def test_send_reminder_notice(mock_settings, mock_fastmail, mock_template_env):
    """Test send_reminder_notice sends email."""
    client = EmailClient()

    await client.send_reminder_notice("test@test.com", "Test Movie", "poster.jpg", days_left=7)
...
@pytest.mark.asyncio
async def test_send_deletion_notice_when_disabled():
    ...
    with patch("scruffy.frameworks_and_drivers.email.email_client.settings") as mock_settings:
        ...
        # Should not raise
        await client.send_deletion_notice("test@test.com", "Test Movie", "poster.jpg")


@pytest.mark.asyncio
async def test_send_reminder_notice_when_disabled():
...
def test_template_path_exists(mock_settings, mock_fastmail, mock_template_env):
    """Test template path is correctly set."""
    client = EmailClient()

    # Verify template path exists (or at least the directory structure)
    template_path = Path(__file__).parent.parent.parent.parent / "scruffy" / "templates"
