...
def test_from_seerr_response_movie():
    """Test from_seerr_response parses movie request correctly."""
    response = {
    ...
    dto = RequestDTO.from_seerr_response(response)
...
def test_from_seerr_response_tv():
    """Test from_seerr_response parses TV request correctly."""
    response = {
...
def test_from_seerr_response_status_mapping():
    """Test from_seerr_response maps statuses correctly."""
    ...
    dto = RequestDTO.from_seerr_response(response_pending)
    assert dto.request_status == RequestStatus.PENDING_APPROVAL
    ...
    dto = RequestDTO.from_seerr_response(response_declined)
    assert dto.request_status == RequestStatus.DECLINED
    assert dto.media_status == MediaStatus.UNKNOWN


def test_from_seerr_response_partially_available():
    """Test from_seerr_response handles partiallyAvailable status."""
    response = {
        "requestedBy": {"id": 1, "email": "test@example.com"},
        "type": "tv",
        "id": 1,
